...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadWriteTx) ([]monautil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	var addrs []monautil.Address
	err := w.Manager.ForEachRelevantActiveAddress(
		addrmgrNs, func(addr monautil.Address) error {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, scope waddrmgr.KeyScope) (monautil.Address, error) {
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb monautil.Amount, label string) (*wire.MsgTx, error) {
