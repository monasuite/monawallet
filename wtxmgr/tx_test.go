...
func assertBalance(t *testing.T, s *Store, ns walletdb.ReadWriteBucket,
	confirmed bool, blockHeight int32, exp monautil.Amount) {
...
func TestOutputLocks(t *testing.T) {
	...
	coinbase := newCoinBase(
		monautil.SatoshiPerBitcoin, monautil.SatoshiPerBitcoin*2,
	)
	coinbaseHash := coinbase.TxHash()

	// One of the spends will be unconfirmed.
	const unconfirmedBalance = monautil.SatoshiPerBitcoin / 2
	...
	const confirmedBalance = monautil.SatoshiPerBitcoin
